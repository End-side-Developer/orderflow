...
const NODE_STYLE: Record<CaseFlowNode["node_type"], string> = {
  ...
  event: "border-slate-400/40 bg-muted0/10",
...
function CaseNodeCard({ data }: NodeProps<Node<CaseNodeData>>) {
  return (
    ...
    >
      <Handle type="target" position={Position.Left} className="!h-2 !w-2 !bg-muted0" />
      ...
      <Handle type="source" position={Position.Right} className="!h-2 !w-2 !bg-muted0" />
    </div>
  );
}
