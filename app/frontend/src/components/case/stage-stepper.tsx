          ...
          return (
            <div key={stage.id} className="flex items-center gap-1 flex-1 last:flex-none">
              <button
                ...
                disabled={isLocked}
                className={`flex items-center gap-2.5 px-3 py-2 rounded-md transition-all duration-200 group relative min-w-max ${
              ...
              >
                ...
                <span className={`text-xs font-semibold tracking-wide uppercase transition-colors ${
                  isActive ? "text-primary" : isLocked ? "" : isCompleted ? "text-foreground" : "text-muted-foreground"
                }`}>
                  {stage.label}
                </span>
