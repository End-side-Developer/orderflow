    ...
    try:
        ...
        if status_code == 429:
            raise GroqQuotaError(
                f"Groq rejected {request_label}: quota or rate limit reached.",
                retry_after_seconds=retry_after,
                provider_detail=detail,
            ) from exc
        if status_code in {401, 403}:
            raise GroqAuthError(
                f"Groq rejected {request_label}. Check ORDERFLOW_AI_GROQ_API_KEY.",
                provider_detail=detail,
            ) from exc
        if status_code == 400:
            raise GroqBadRequestError(
                f"Groq rejected {request_label}. Check ORDERFLOW_AI_DEFAULT_MODEL.",
                provider_detail=detail,
            ) from exc
        if 500 <= status_code < 600:
            raise GroqServerError(
                f"Groq service returned HTTP {status_code} for {request_label}.",
                provider_detail=detail,
            ) from exc
        raise GroqError(
        ...
        raise GroqTimeoutError(
            f"Groq timed out after {settings.orderflow_ai_timeout_seconds}s for {request_label}."
        ) from exc
    except httpx.RequestError as exc:
        raise GroqNetworkError(f"Network error calling Groq for {request_label}: {exc}") from exc
    except Exception as exc:
        raise GroqError(f"Groq request failed for {request_label}: {exc}") from exc
