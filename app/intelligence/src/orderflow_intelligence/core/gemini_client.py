...
# ──────────────────────────────────────────────────────────────────────────────
# Structured Gemini error taxonomy (mirror of orderflow_api.core.gemini_client)
#
# The two clients are intentionally kept in lock-step so the LangGraph extractor
# can surface the same error codes the FastAPI routes do. If you change one,
# change the other.
# ──────────────────────────────────────────────────────────────────────────────
        ...
        while True:
            with self._lock:
                ...
                wait_seconds = max(
                    ...
                    self._token_wait_seconds(now, tokens),
                )
                if wait_seconds <= 0:
                    self._sequence += 1
            ...
            if time.monotonic() + wait_seconds > deadline:
                raise GeminiQuotaError(
                    "Gemini minute quota is saturated and would require waiting longer than "
...
def extract_gemini_text(response: dict[str, object]) -> str:
    ...
    if not isinstance(candidates, list) or not candidates:
        ...
        raise GeminiEmptyResponseError("Gemini response missing candidates")
        ...
        for reason in finish_reasons:
            ...
            if reason == "MAX_TOKENS":
                raise GeminiEmptyResponseError(
                    "Gemini hit max_output_tokens before producing any text."
                )
        raise GeminiEmptyResponseError("Gemini response text is empty")
...
def _classify_http_error(exc: urllib_error.HTTPError, label: str) -> GeminiError:
    ...
    if exc.code == 429 or "RESOURCE_EXHAUSTED" in detail_upper:
        return GeminiQuotaError(
            f"Gemini rejected the {label} request because quota was exhausted: {detail}",
            provider_detail=short_detail,
        )
    if exc.code in {401, 403}:
        return GeminiAuthError(
            f"Gemini rejected the {label} request: API key is missing or invalid.",
            provider_detail=short_detail,
        )
    if exc.code == 400:
    ...
    return GeminiError(
        f"Gemini returned HTTP {exc.code} on the {label} request.",
        provider_detail=short_detail,
    )
    ...
    *,
    ...
    try:
        ...
        raise GeminiTimeoutError(
            f"Gemini did not respond within {settings.orderflow_ai_timeout_seconds}s for {request_label}.",
        ) from exc
        ...
        if is_timeout:
            raise GeminiTimeoutError(
                f"Network timeout calling Gemini for {request_label}: {reason}",
            ) from exc
        raise GeminiNetworkError(
            f"Network error calling Gemini for {request_label}: {reason}",
        ) from exc
